...
### Module Boundary (모듈 경계) — CRITICAL
...
- 사용자가 "점검"을 요청하면 **개발 철학 13대 원칙**을 기준으로 해당 도메인의 코드/설계를 점검하고 결과를 보고한다.
...
info("회원 생성 완료: memberId="+memberId +", role="+role);
```
...
### Response DTO Composition (응답 구조화) — CRITICAL
...
```java
where(MemberSpec.isActive(active),MemberSpec.
...
### DDD Bounded Context + Vertical Slicing — CRITICAL
...
| Response 구조화  | 필드 나열 금지, 의미 단위 DTO로 묶어 조합 (3개 이하 단순 응답은 예외)                                                         |
...
| 의존성 문서        | `build.gradle.kts` 의존성 변경 시 `docs/backend/BACKEND_DEPENDENCIES.md`도 반드시 함께 갱신                        |
