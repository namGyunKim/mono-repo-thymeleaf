...
>
> - **자율 진행은 무슨 일이 있어도 `git commit`까지만이다. `git push`, PR 생성, PR 머지는 절대로 사용자의 명시적 요청 없이 수행하지 않는다.**
> - 사용자가 **"커밋해줘"** 라고 하면 **커밋까지만** 수행한다. push/PR은 절대 포함하지 않는다.
> - 사용자가 **"PR 해줘"** 라고 명시적으로 말한 경우에만 push + PR을 수행한다.
> - 이전 대화에서 push/PR을 수행한 적이 있더라도 **매 요청마다 독립적으로 판단**한다 — 관성으로 행동하지 않는다.
> - 작업 흐름상 push/PR이 자연스러워 보여도 **사용자가 요청하지 않았으면 하지 않는다.** 절대 "편의상" 또는 "당연히 다음 단계니까" 라는 이유로 push/PR을 자율 수행하지 않는다.
